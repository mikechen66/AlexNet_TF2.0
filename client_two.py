...
"""
Use the command "run_experiment(n, large_data_set=True, generator=True)" to initiate the functional call. It saves
the intermediate big data to make full use of the RAM(Random Access Memory).
"""
...
def run_experiment(n, large_data_set=False, generator=False):
    ...
    for experiments in range(n):
        ...
        # -model = AlexNet(train_data, test_data, val_data, generator=True)  
        model = AlexNet(train_data, test_data, val_data)           
...
run_experiment(n, large_data_set=True, generator=True)
